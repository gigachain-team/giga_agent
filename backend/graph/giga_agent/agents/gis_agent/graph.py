...
async def city_explore(city: str):
    ...
    return {
        "data": data_message,
        "message": f'В результате была получена информация о городе и страница с картой {uploaded["path"]}. Напиши пользователю историю города, пару фактов о нем. Покажи карту пользователю через "![alt-описание](attachment:{uploaded["path"]})". Приложи ВСЮ информацию о достопримечательностях, отелях, и где покушать. Также обязательно прикладывай изображения к отелям и кафе, если они есть в конкретном месте!, в формате ![alt-текст](url)',
